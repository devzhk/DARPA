...
class SpectralConv2d(nn.Module):
    ...
    def forward(self, x, gridy=None):
        ...
        if gridy is None:
            ...
            x = self.ifft2d(gridy, factor1, factor2, self.modes1,
                            self.modes2) / (size1 * size2)
        return x
    ...
    def ifft2d(self, gridy, coeff1, coeff2, k1, k2):
        ...
        coeff4 = torch.cat([coeff1[:, :, 0:1, 1:].flip(-1).conj(),
                           coeff2[:, :, :, 1:].flip(-1, -2).conj()], dim=-2)
...
class SpectralConv3d(nn.Module):
    ...
    def forward(self, x):
        ...
        # Multiply relevant Fourier modes
        out_ft = torch.zeros(batchsize, self.out_channels, x.size(2), x.size(
            3), x.size(4)//2 + 1, device=x.device, dtype=torch.cfloat)
        out_ft[:, :, :self.modes1, :self.modes2, :self.modes3] = \
            compl_mul3d(x_ft[:, :, :self.modes1, :self.modes2,
        ...
        out_ft[:, :, -self.modes1:, -self.modes2:, :self.modes3] = \
            compl_mul3d(x_ft[:, :, -self.modes1:, -
                        self.modes2:, :self.modes3], self.weights4)
